...
class YB_Pcb_Car(object):
    ...
    def __init__(self):
        # Create I2C device.
        DEVICE_ADDRESS = 0x16
        self._device = self.get_i2c_device(DEVICE_ADDRESS, 1)

    def write_u8(self, reg, data):
        try:
            print(self._addr)
            #self._device.read_byte(self._addr)
    ...
    def write_array(self, reg, data):
        ...
        try:
            ...
            Rdata= self._device.read_byte(self._addr)
            print(f"device {hex(self._addr)} connect. data: {Rdata}")
            
            self._device.write_i2c_block_data(self._addr, reg, data)
            #Rdata= self._device.read_byte(self._addr, reg)
            #print(f"Data read from device: {Rdata}")
        except:
    ...
    def Car_Stop(self):
        try:
            #print(smbus.SMBus(i2c_bus))
            reg = 0x02
            print('Car Stop', reg)
            self.write_u8(reg, 0x00)
            #self.Ctrl_Car(1, 0, 1, 0)
        except:
            print ('Car_Stop I2C error')

    def Car_Back(self, speed1, speed2):
        try:
            #data= self._device.read_byte(self._addr)
            print ('Car back')
